programa
{
	...
	funcao inicio()
	{
		 ...
		 */
			...
			real pesoTomate, multaTotal, precoTotal
			real pesoExcedente = 0.00
			const inteiro pesoLimite = 50
			const real multaValor = 4.00

				escreva("Peso comprado: ")
				leia(pesoTomate)

					se(pesoTomate <= pesoLimite){
						multaTotal= 0.00
						escreva("Peso excedido: 0 kg \n", "Valor multa: R$ 0.00 \n")}
						
					senao se (pesoTomate > pesoLimite){
						pesoExcedente = (pesoTomate - pesoLimite)
						multaTotal = (pesoExcedente * multaValor)
						escreva("Peso excedido: ", pesoExcedente, " kg \n", "Multa: R$ ", multaTotal, "\n")}
